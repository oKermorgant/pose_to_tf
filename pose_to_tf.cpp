...
public:
    ...
    child_frame   {declare_parameter<string>("child_frame", "base_link")}
  {
    ...
    if(topic[0] != '/')
      topic = string(get_namespace()) + "/" + topic;
...
private:
  ...
  inline Header toHeader(const string &parent_frame)
  {
    Header header;
  ...
  void republish(const Header &header,
  ...
  void detectTopicType()
  {
    ...
    else if(msg == "geometry_msgs/msg/TransformStamped")
    {
      ...
      {republish(msg->header, msg->child_frame_id, msg->transform);});
    }
